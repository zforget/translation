## 第五章 记录
OCaml最好的特性之一就是其简捷且富有表达力的用以声明新数据类型的系统，记录是这个系统的一个关键要素。在[第一章导览](https://github.com/zforget/translation/blob/master/real_world_ocaml/1_01_a_guide_tour.md)中我们简要讨论了一下记录，但本章会更深入，覆盖记录类型的工作原理，以及如何在软件设计中有效使用它们的建议。
...
OCaml默认不可变的策略是好的，但命令式编程也是OCaml编程的一个重要部分。我们会在[“命令式编程”一节](https://github.com/zforget/translation/blob/master/real_world_ocaml/1_08_imperative_programming.md)深入讨论如何（以及何时）使用OCaml的命令式特性。
